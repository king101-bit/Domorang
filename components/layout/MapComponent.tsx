  ...
  return (
      <>
       {/* Search Bar */}
        <div className="mx-auto mb-8 max-w-2xl">
          <div className="flex overflow-hidden rounded-full bg-gray-100 shadow-md">
            ...
            <Link href="/properties">
              <Button 
                size="lg"
                className="rounded-l-none rounded-r-full bg-primary-600 px-8 hover:bg-primary-700"
              >
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
                {popularAreas.slice(0, 4).map((area) => (
                  ...
                  >
                    <MapPin className="mx-auto mb-1 h-4 w-4 text-primary-600" />
                    <p className="text-sm font-semibold">{area.name}</p>
                    <p className="text-xs text-gray-600">{area.count} properties</p>
                  </Link>
                ))}
              </div>
            </div>
        ...
        <div className="mt-10">
          <h3 className="mb-6 text-center text-2xl font-bold">Popular Neighborhoods</h3>
          <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
            {popularAreas.map((area) => (
              <Link
                key={area.name}
                href={`/properties?area=${area.name.toLowerCase()}`}
                className="rounded-xl border-2 border-gray-200 bg-white p-4 text-center transition-all hover:border-primary-600 hover:shadow-lg"
              >
